...
func main() {
	...
	flag.Parse()
    if *help || flag.NArg() != 1 {
		fmt.Printf("USAGE: nested <string>\n")
		flag.PrintDefaults()
    	return;
    }

	debug := utils.NewDebug(utils.USER,"nested")
	...
	nobuff := utils.MakeByteOEncoding(2048)
	e = nobuff.WriteString(flag.Arg(0))
	if e != nil {
		fmt.Printf("Error: %s\n",e)
		os.Exit(1)
	}
	e = nobuff.WriteBinary([]byte(flag.Arg(0)))
	if e != nil {
		fmt.Printf("Error: %s\n",e)
		os.Exit(1)
	}
	b,e := nobuff.GetBuffer()
	if e != nil {
		fmt.Printf("Error: %s\n",e)
		return
	}
	...
	debug.Printf("Sending structued data {T1,B}")
	e = obuff.WriteStructured(b)
	if e != nil {
		fmt.Printf("Error: %s\n",e)
		os.Exit(1)
	}
