...
func worker(name string,conn *net.TCPConn) {
	...
	b,e := ibuff.ReadStructured()
	if e != nil {
		fmt.Printf("%s Error: %s\n",name,e)
		return
	}
	debug.PrintBuffer(b,"Encoding for T1,B = ")

	nibuff := utils.MakeByteIEncoding(b)
	debug.Printf("Reading string T1")
	s,e := nibuff.ReadString()
	if e != nil {
		fmt.Printf("%s Error: %s\n",name,e)
		return
	}
	...
	debug.Printf("Sending string T2")
	e = obuff.WriteString("God is alive and well, and working on a less ambitious project")
	if e != nil {
		fmt.Printf("%s Error: %s\n",name,e)
		return
	}
...
func init() {
    workers.AddWorker("nested",1,worker);
}
